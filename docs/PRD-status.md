...
| DM-01–04 | Prefix search with minimum length and rate limit, unique sorted pair, incoming/sent requests, accept/decline/cancel/block, inbox, text/media after acceptance, blocking and search/presence hiding | Existing direct channels are migrated as accepted; inbox sorts server-side by last send activity; configurable request privacy preferences remain deferred |
...
## Profile and room pictures

Avatars and room icons accept JPEG/PNG/WebP up to 5 MB and 24 million source pixels. Images are oriented, center-cropped to 256×256, stripped of metadata and saved as static WebP. Thumbnails are stored in separate database tables with cascading owner/room deletion; CDN/object storage is still deferred. Avatar reads require sign-in and respect blocking. Private room icons require membership; only owners/admins can upload or remove room icons. The editor provides preview, save, cancel, replacement and removal. Pictures update live for room members and direct-message contacts, and refresh after reconnecting. A full browser/device accessibility matrix remains unverified.
