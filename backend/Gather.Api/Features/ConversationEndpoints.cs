...
public static class ConversationEndpoints
{
    public static void MapConversations(this WebApplication app)
    {
        ...
        api.MapGet("/dm", async (ClaimsPrincipal p, GatherDb db) => { var id = p.UserId(); return await (from c in db.Channels join u in db.Users on (c.UserLow == id ? c.UserHigh : c.UserLow) equals u.Id where c.RoomId == null && (c.UserLow == id || c.UserHigh == id) && db.DmRequests.Any(r => r.ChannelId == c.Id && r.State == "Accepted") orderby c.LastActivity descending select new { channelId = c.Id, user = new { u.Id, u.Username, u.DisplayName, u.Bio }, blocked = db.Blocks.Any(b => (b.UserId == id && b.TargetId == u.Id) || (b.UserId == u.Id && b.TargetId == id)), unread = db.Messages.Count(m => !m.Deleted && m.ChannelId == c.Id && m.SenderId != id && string.Compare(m.Id, db.Reads.Where(s => s.UserId == id && s.ChannelId == c.Id).Select(s => s.LastMessageId).FirstOrDefault() ?? "") > 0) }).ToListAsync(); });
